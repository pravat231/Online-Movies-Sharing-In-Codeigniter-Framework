...
class Favorites extends CI_Controller {
	...
	function Index()
	{
		...
		$data['site_title']=$this->uri->segment(3).'\'s Profile on Go4film.com - Favorites';
		...
		$index_data['stage_nav']='<h1 class="link"><a href="'.base_url().'profile/'.$user_name.'">'.$user_name.'\'s</a></h1> > <h1>Favorites</h1>';
		...
		$this->home_model->Set_free_where();	
		$this->home_model->Set_sql("gf_film.film_id,film_name,film_post_link,website_poster_url,favorite_id,DATE_FORMAT(added_on,'%b %d, %Y') AS added_on FROM gf_film LEFT JOIN gf_film_poster ON gf_film.film_id=gf_film_poster.film_id LEFT JOIN gf_favorites ON gf_favorites.film_id=gf_film.film_id");
		$this->home_model->Set_where("gf_favorites.user_id='".$profile['row_profile']->user_id."'");
		...
		$this->load->view('profile/favorite_post',$index_data);			
