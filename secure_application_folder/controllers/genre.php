...
class Genre extends CI_Controller {
	...
	function Index()
	{
		$genre_name=$this->uri->segment(2); 
		if(!preg_match('/[a-z]/',$genre_name)){
			redirect(base_url());
			exit(0);
		}
		...
		if($this->uri->segment(3)=='sort')
		{
			...
			$page['base_url'] = base_url().'genre/'.$genre_name.'/sort/'.$this->uri->segment(4).'/page/';
		}
		else
		{
			$offset=(int)$this->uri->segment(4);
			$page['base_url'] = base_url().'genre/'.$genre_name.'/page/';
		}
		if($offset<1) 
		{ 
			$offset=1; 
		}
		$genre_id=$this->home_model->genre_id($genre_name);
		if($offset==1)
		{
			$data['site_title']=ucfirst($genre_name).' Movies - Watch Onlline Movies Free';
		}
		else
		{
			$data['site_title']=ucfirst($genre_name).' Movies - Watch Onlline Movies Free - Page-'.$offset;
		}
		...
		$index_data['stage_nav']='<div class="link_nowrap">'.ucfirst($genre_name).' Movies</div> > <div class="link_wrap">'.$this->sorted->sort_by_title().'</div>';
		...
		$this->home_model->Set_free_where();	
		$this->home_model->Set_sql("gf_film.film_id,film_name,DATE_FORMAT(film_release_date,'%d') AS DATE,DATE_FORMAT(film_release_date,'%m') AS month_ori,DATE_FORMAT(film_release_date,'%M') AS MONTH,DATE_FORMAT(film_release_date,'%Y') AS YEAR,film_release_date,film_feature,film_modify,film_post_link,website_poster_url,film_genre_value AS genres FROM gf_film LEFT JOIN gf_film_genre ON gf_film_genre.film_id=gf_film.film_id LEFT JOIN gf_genre ON gf_film_genre.genre_id=gf_genre.genre_id");
		$this->home_model->Set_where("gf_film_genre.genre_id='".$genre_id."'");
