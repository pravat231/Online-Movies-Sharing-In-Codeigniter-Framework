...
class Director extends CI_Controller {
	...
	function Index()
	{
		$director_id=$this->uri->segment(2); 
		if(!preg_match('/[0-9]/',$director_id)){
			redirect(base_url());
			exit(0);
		}
		...
		if($this->uri->segment(3)=='sort')
		{
			...
			$page['base_url'] = base_url().'director/'.$director_id.'/sort/'.$this->uri->segment(4).'/page/';
		}
		else
		{
			$offset=(int)$this->uri->segment(4);
			$page['base_url'] = base_url().'director/'.$director_id.'/page/';
		}
		if($offset<1) 
		{ 
			$offset=1; 
		}
		$director_name=$this->home_model->director_name($director_id);
		if($offset==1)
		{
			$data['site_title']=$director_name.'\'s Movies - Watch Onlline Movies Free';
		}
		else
		{
			$data['site_title']=$director_name.'\'s Movies - Watch Onlline Movies Free - Page-'.$offset;
		}
		...
		$index_data['stage_nav']='<div class="link_nowrap">'.ucfirst($director_name).'\'s Movies</div> > <div class="link_wrap">'.$this->sorted->sort_by_title().'</div>';
		...
		$this->home_model->Set_free_where();	
		$this->home_model->Set_sql("gf_film.film_id,film_name,DATE_FORMAT(film_release_date,'%d') AS DATE,DATE_FORMAT(film_release_date,'%m') AS month_ori,DATE_FORMAT(film_release_date,'%M') AS MONTH,DATE_FORMAT(film_release_date,'%Y') AS YEAR,film_release_date,film_feature,film_modify,film_post_link,website_poster_url,film_genre_value AS genres FROM gf_film left join gf_film_director on gf_film_director.film_id=gf_film.film_id");
		$this->home_model->Set_where("gf_film_director.director_id='".$director_id."'");
